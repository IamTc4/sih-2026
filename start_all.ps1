...
# ── Helper: start a service in a new Windows Terminal / PowerShell window ──
function Start-Service-Window {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command",
        "cd '$Dir'; Write-Host '=== $Name ===' -ForegroundColor $Color; $Command"
}

# ── 1. Blockchain Service (port 8000) ──────────────────────────────────────
Start-Service-Window `
    -Name "Blockchain (port 8000)" `
    -Dir  "$ROOT\blockchain" `
...
# Train first if model.pkl doesn't exist
Start-Service-Window `
    -Name "ML Risk Scoring (port 8002)" `
    -Dir  "$ROOT\ML" `
    -Command "if (-not (Test-Path 'ml\data\model.pkl')) { Write-Host 'Training ML model...' -ForegroundColor Yellow; python train.py }; python main.py" `
    -Color "Magenta"
...
Start-Service-Window `
    ...
    -Dir  "$ROOT\Dashboard" `
    -Command "if (-not (Test-Path 'node_modules')) { Write-Host 'Installing packages...' -ForegroundColor Yellow; npm install }; npm run dev" `
    -Color "Yellow"
