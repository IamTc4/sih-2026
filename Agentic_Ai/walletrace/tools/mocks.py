...
def draft_legal_notice(case_id: str, exchange: str, evidence_summary: str) -> dict:
    ...
    draft_text = (
        f"═══════════════════════════════════════════════════════════════════════════════\n"
        f"   OFFICE OF THE SUPERINTENDENT OF POLICE / CYBER CRIME INVESTIGATION CELL\n"
        f"        CENTRAL NATIONAL CYBERCRIME REPORTING PORTAL (NCRP) / MHA I4C\n"
        f"═══════════════════════════════════════════════════════════════════════════════\n\n"
        f"NOTICE UNDER SECTION 91 READ WITH SECTION 102 OF THE CODE OF CRIMINAL PROCEDURE, 1973\n"
        f"(CORRESPONDING TO SECTION 94 & 107 OF BHARATIYA NAGARIK SURAKSHA SANHITA, 2023)\n"
        f"& SECTION 66B / 66C / 66D OF THE INFORMATION TECHNOLOGY (AMENDMENT) ACT, 2008\n\n"
        f"Notice Reference No. : NCRP/CRPC-91/{case_id}\n"
        f"Issue Date & Time    : {now_str}\n"
        f"Investigating Unit   : Cyber Crime Police Station (Financial Crimes & Crypto Forensics Cell)\n\n"
        f"TO:\n"
        f"The Nodal / Designated Compliance Officer,\n"
        f"{exchange}\n"
        f"(Registered Virtual Digital Asset Service Provider / FIU-IND Reporting Entity)\n\n"
        f"SUBJECT: URGENT STATUTORY REQUISITION FOR SUBSCRIBER KYC, TRANSACTION AUDIT TRAILS,\n"
        f"         IP LOGS, AND IMMEDIATE FREEZING OF SUSPECT WALLET/ACCOUNTS\n\n"
        f"Sir / Madam,\n\n"
        f"WHEREAS, an investigation has been initiated into an alleged cyber fraud / financial\n"
        f"syndicate under Case/FIR Ref: {case_id}, involving wrongful diversion and laundering\n"
        f"of cryptocurrency (USDT-TRC20 on Tron network).\n\n"
        f"AND WHEREAS, through automated multi-hop blockchain forensic tracing and heuristic\n"
        f"clustering, the stolen funds have been traced and conclusively identified as having\n"
        f"terminated into deposit infrastructure owned and/or operated by your platform ({exchange}).\n\n"
        f"EVIDENCE OF ON-CHAIN FUND FLOW & ATTRIBUTION:\n"
        f"───────────────────────────────────────────────────────────────────────────────\n"
        f"{evidence_summary}\n"
        f"───────────────────────────────────────────────────────────────────────────────\n\n"
        f"YOU ARE HEREBY DIRECTED PURSUANT TO SECTION 91 & SECTION 102 CrPC / BNSS TO FURNISH\n"
        f"THE FOLLOWING RECORDS WITHIN TWENTY-FOUR (24) HOURS OF RECEIPT OF THIS NOTICE:\n\n"
        f"1. Complete KYC Dossier: Full Legal Name, Date of Birth, Permanent Address, Certified\n"
        f"   Identity Proof (PAN / Aadhaar / Passport), Registered Mobile Number, and Email ID.\n"
        f"2. Fiat Banking Links: Bank Account Numbers, Account Holder Name, IFSC Code, and\n"
        f"   associated UPI VPA IDs linked to this account.\n"
        f"3. Detailed Transaction History: Full ledger of all crypto-to-crypto and crypto-to-INR/fiat\n"
        f"   conversions, P2P trade counterparty records, and off-chain transfer logs.\n"
        f"4. Access Logs & Device Telemetry: Complete IP address login history with port numbers,\n"
        f"   MAC addresses, IMEI/Device Fingerprints, and timestamped session records.\n"
        f"5. IMMEDIATE FREEZING ORDER: In exercise of powers under Section 102 CrPC, you are\n"
        f"   directed to immediately FREEZE all current balances (crypto and fiat) associated\n"
        f"   with the identified user account(s) and prevent any outward remittance.\n\n"
        f"TAKE NOTICE that intentional omission, non-compliance, or destruction of requested\n"
        f"electronic evidence shall attract penal prosecution under Sections 175, 176, and 201\n"
        f"of the Indian Penal Code, 1860 / Bharatiya Nyaya Sanhita, 2023, and provisions of PMLA 2002.\n\n"
        f"───────────────────────────────────────────────────────────────────────────────\n"
        f"LEGAL STATUS: [ DRAFT - REQUIRES INVESTIGATOR REVIEW & DIGITAL SIGNATURE ]\n"
        f"Generated via WalletTrace Automated Forensics Engine (SIH26183 Compliance)\n\n"
        f"INVESTIGATING OFFICER (IO):\n"
        f"Name / Badge No.    : __________________________________________\n"
        f"Designation / Rank  : Inspector of Police / Cyber Forensics Investigator\n"
        f"Police Station      : __________________________________________\n"
        f"Official Seal       : [AFFIX POLICE SEAL HERE]\n"
    )
