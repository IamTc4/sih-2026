...
export default function ExchangePanel({ traceResult, loading }) {
      ...
      {attr && (
        <div className="space-y-space-sm">
          <div className="flex items-center justify-between">
            <div>
              {attr.status === 'KNOWN' ? (
                <div className="font-headline-md text-headline-md font-bold text-primary">{attr.exchange_name}</div>
              ) : (
            ...
            <span className={`font-label-caps text-label-caps px-space-xs py-space-2xs rounded border ${
              attr.status === 'KNOWN'
          ...
          <div>
            <div className="flex justify-between font-code-sm text-code-sm text-on-surface-variant mb-space-2xs">
              ...
              <span className="text-primary">{(attr.confidence * 100).toFixed(0)}%</span>
            </div>
            <div className="h-1.5 bg-surface-container-high rounded-full overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-primary to-secondary"
                style={{ width: `${attr.confidence * 100}%`, transition: 'width 0.8s ease' }}
              />
            </div>
          </div>
